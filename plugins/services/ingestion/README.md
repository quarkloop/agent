...
Each source record tracks `file_path`, `source_hash`, extraction state, LLM
structuring state, embedding state, indexing state, citation state, and
`last_error`. Agents resume only pending or failed sources returned by
`ingestion_ListIncompleteSources` or incomplete sources in `ingestion_ResumeRun`.
Succeeded sources are not repeated unless the user explicitly asks to re-index.
