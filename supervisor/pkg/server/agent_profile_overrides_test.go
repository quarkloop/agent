...
func TestAgentProfileOverrideResolverAppliesQuarkfileOverrides(t *testing.T) {
	...
	qf := &spacemodel.Quarkfile{
		...
		Agents: []spacemodel.AgentRef{{
			...
			Enabled:  &enabled,
			Services: []string{"io_Read", "indexer_QueryContext", "gateway_Embed"},
	...
	if !sameStrings(profile.Permissions.Services, []string{"io_Read", "indexer_QueryContext", "gateway_Embed"}) {
	...
	return runtimePluginCatalogEntry{
		...
		AgentProfile: &plugin.AgentProfile{
			...
			Permissions: plugin.AgentProfilePermission{
				Services: []string{"io_Read", "io_Execute", "indexer_QueryContext", "gateway_Embed", "document_ExtractText"},
			},
