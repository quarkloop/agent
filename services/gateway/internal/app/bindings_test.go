...
func TestGatewayBindingDispatchesUnaryOperation(t *testing.T) {
	...
	host, err := natskit.StartRPCService(context.Background(), natskit.Config{URL: broker.ClientURL(), Name: "gateway-host"}, gatewayBinding(nil, server))
	if err != nil {
...
func TestGatewayBindingStreamsSingleTerminalCompletion(t *testing.T) {
	broker := startGatewayNATS(t)
	server := gatewayServer(t)
	host, err := natskit.StartRPCService(context.Background(), natskit.Config{URL: broker.ClientURL(), Name: "gateway-stream-host"}, gatewayBinding(nil, server))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(host.Close)
