...
import (
	...
	"github.com/quarkloop/services/runstate/internal/app"
)
...
func main() {
	...
	flag.StringVar(&addr, "addr", "127.0.0.1:7308", "service descriptor address")
	flag.StringVar(&root, "root", os.Getenv("QUARK_RUNSTATE_ROOT"), "Run State service durable record directory")
	...
	flag.StringVar(&natsQueue, "nats-queue", envOrDefault("QUARK_RUNSTATE_NATS_QUEUE", "q.runstate.v1"), "NATS queue group")
	flag.Parse()

	logger := slog.New(slog.NewTextHandler(os.Stderr, nil).WithAttrs([]slog.Attr{
		slog.String("process", "service"),
		slog.String("service", "runstate"),
	}))
