...
func (p *bifrostProvider) ListModels(ctx context.Context) ([]*modelv1.ModelInfo, error) {
	if p.client == nil {
	...
	out := make([]*modelv1.ModelInfo, 0, len(resp.Data))
	for _, model := range resp.Data {
		...
		out = append(out, &modelv1.ModelInfo{
			Id:            model.ID,
			Provider:      p.id,
			Name:          name,
	...
	if len(out) == 0 && p.model != "" {
		out = append(out, &modelv1.ModelInfo{Id: p.model, Provider: p.id, Name: p.model, DefaultModel: true})
	}
	return out, nil
}
...
func (p *bifrostProvider) Embed(ctx context.Context, cmd embedCommand) ([]*modelv1.Embedding, error) {
	if p.client == nil {
		return nil, plugin.NewProviderError(plugin.ProviderErrorTransport, p.id, cmd.Model, 0, fmt.Errorf("bifrost client is not initialized"))
	}
	format := "float"
	...
	out := make([]*modelv1.Embedding, 0, len(resp.Data))
