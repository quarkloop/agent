...
func (p *localProvider) ListModels(context.Context) ([]*modelv1.ModelInfo, error) {
...
func (p *localProvider) Embed(_ context.Context, cmd embedCommand) ([]*modelv1.Embedding, error) {
	...
	out := make([]*modelv1.Embedding, 0, len(cmd.Input))
...
func rerankLocal(query string, documents []string) []*modelv1.RerankResult {
	queryTerms := termSet(query)
	results := make([]*modelv1.RerankResult, 0, len(documents))
	for i, doc := range documents {
		...
		results = append(results, &modelv1.RerankResult{Index: int32(i), Score: score})
	}
