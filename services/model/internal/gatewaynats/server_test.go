...
func TestGatewayNATSGenerateAndUsageSummary(t *testing.T) {
	ns := startTestNATS(t)
	srv, err := modelsvc.NewServer(modelsvc.Config{
		Providers: []modelsvc.ProviderConfig{{ID: "local", Kind: "local", Model: "local/noop", Enabled: true}},
	})
	if err != nil {
	...
	var generated modelv1.GenerateResponse
	callGateway(t, conn, "generate", &modelv1.GenerateRequest{
		Provider: "local",
		Model:    "local/noop",
		Messages: []*modelv1.ModelMessage{{Role: "user", Content: "say hello"}},
	}, &generated)
	if generated.GetText() == "" || generated.GetUsage().GetProvider() != "local" {
		t.Fatalf("generate response = %+v", &generated)
	}

	var summary modelv1.UsageSummaryResponse
	callGateway(t, conn, "usage_summary", &modelv1.UsageSummaryRequest{}, &summary)
...
func TestGatewayNATSReloadConfig(t *testing.T) {
	...
	conn := connectTestNATS(t, ns.ClientURL())
	defer conn.Close()
	var reloaded modelv1.ReloadConfigResponse
	callGateway(t, conn, "reload_config", &modelv1.ReloadConfigRequest{
	...
	var models modelv1.ListModelsResponse
	callGateway(t, conn, "list_models", &modelv1.ListModelsRequest{Provider: "local"}, &models)
...
func TestGatewayNATSStreamGenerate(t *testing.T) {
	...
	payload, err := protojson.Marshal(&modelv1.StreamGenerateRequest{
		Provider: "local",
		Model:    "local/noop",
		Messages: []*modelv1.ModelMessage{{Role: "user", Content: "stream this"}},
	})
	if err != nil {
