...
func main() {
	...
	flag.StringVar(&natsQueue, "nats-queue", envOrDefault("QUARK_INGESTION_NATS_QUEUE", "q.ingestion.v1"), "NATS queue group")
	flag.Parse()

	logger := slog.New(slog.NewTextHandler(os.Stderr, nil).WithAttrs([]slog.Attr{
		slog.String("process", "service"),
		slog.String("service", "ingestion"),
	}))
