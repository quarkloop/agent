...
func main() {
	...
	flag.StringVar(&addr, "addr", "127.0.0.1:7302", "service descriptor address")
	...
	flag.StringVar(&natsQueue, "nats-queue", envOrDefault("QUARK_BUILD_RELEASE_NATS_QUEUE", "q.build_release.v1"), "NATS queue group")
	flag.Parse()

	logger := slog.New(slog.NewTextHandler(os.Stderr, nil).WithAttrs([]slog.Attr{
		slog.String("process", "service"),
		slog.String("service", "build-release"),
	}))
