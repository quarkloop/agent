...
import (
	...
	"github.com/quarkloop/services/devops/internal/app"
)
...
func main() {
	...
	flag.StringVar(&natsQueue, "nats-queue", envOrDefault("QUARK_DEVOPS_NATS_QUEUE", "q.devops.v1"), "NATS queue group")
	flag.Parse()

	logger := slog.New(slog.NewTextHandler(os.Stderr, nil).WithAttrs([]slog.Attr{
		slog.String("process", "service"),
		slog.String("service", "devops"),
	}))
